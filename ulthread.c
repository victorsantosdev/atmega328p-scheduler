...
void swapcontext(ULThread * old_thread, ULThread * new_thread) {
    ...
    if (new_thread != 0) {
        //printf("Restoring new_context : %x \n", new_context);
...
void yield(void) {
    ...
    if (sch.ready.size > 0) { //uma tarefa running e uma+ ready
        ...
        } else printf("prioridade do running menor que do head da lista de ready\n");
    }
}
...
void reschedule(void) {
    ...
    if (sch.ready.size > 0) { //uma tarefa running e uma+ ready
        if (sch.running->priority >= sch.ready.head->priority) {
            ...
            printf("prioridade da running menor do que a head da lista\n");
    }
    //resumeScheduler();
